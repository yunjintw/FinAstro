...
function dataReadComplete(){
    updateButton.innerHTML = "Update Graph"
...
function  fillInSelectors() {
    ...
    for (item in stockdisplay_list){
        document.getElementById("stock-display-type").innerHTML += '<option value="' + stockdisplay_list[item] + '">' + stockdisplay_list[item] + '</option>'
    }
    ...
    for (item in bodyDataType_list){
        document.getElementById("body-data-type").innerHTML += '<option value="' + bodyDataType_list[item] + '">' + bodyDataType_list[item] + '</option>'
    }
...
const readDataFile  = async (name, type) => {
    try {
        console.log("in reasdStockFile " + name)
        const res = await fetch("static/data/" + name + ".csv");
...
function addBody(line) {
    ...
    if (item[1] != undefined && item[1] != "object" ) {
        ...
        document.getElementById("body-list").innerHTML += '<label for="' + item[1] +'"> ' + item[1] +'</label>'
...
const loadStocks = async () => {
    try {
        const res = await fetch("static/data/symbols.csv");
        const data = await res.text();
        lines = data.split(/\r?\n/);
    ...
    document.getElementById("stock-select").innerHTML += '<option value="none">none</option>'
};

const loadBodies = async () => {
    try {
        const res = await fetch("static/data/body.csv");
        const data = await res.text();
        lines = data.split(/\r?\n/);
...
function presentGraph() {
    ...
    if (stockname != 'none') {    
        ...
        stockname = 'Solar system bodies only'
        stockgraph = ""
    }
    
    layout = {
        title: {text: stockname + ' ' + stockgraph +' graph'},
        xaxis: {
            title: 'Date',
